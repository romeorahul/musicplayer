...
function Rightsidebar() {
  ...
  useEffect(() => {
    if (!loading && !error) {
      setSongList(data.getSongs || []); // Ensure songList is initialized as an empty array
      setIsPlaying(false);
    }
  }, [data, loading, error, selectedItem]);
  ...
  useEffect(() => {
    if (currentSongIndex < 0 || currentSongIndex >= songList.length) {
      // Check if currentSongIndex is out of bounds
      setCurrentSongIndex(0);
    }
  }, [currentSongIndex, songList]);
  ...
  return (
    <div className={styles.container}>
      {songList.length > 0 && (
        <>
          <h1>{songList[currentSongIndex]?.title}</h1> {/* Use optional chaining */}
          <p>{songList[currentSongIndex]?.artist}</p>
